...
def gdf_to_yolo(geodataframe, image, output_dir, column='single_id',
    ...
    """Convert a polygon gdf to yolo/yolt readable format (.txt files).

    Arguments
    ---------
    geodataframe : str
        Path to a :class:`geopandas.GeoDataFrame` with a
        column named ``'geometry'``.  Can be created from a geojson with labels
        for unique objects. Can be converted to this format with
    ...
    im_size : tuple
        ...
        (0,0) this is automatically extracted from the image.
    min_overlap : float
        A float value ranging from 0 to +1.  This is a percantge.  If a polygon
        does not overlap the image by at least min_overlap, the polygon is
        discarded.  i.e. 0.66 = 66%, at least 66% of the polygon must intersect
        with the image or else it is discarded. Default value of 0.66.
    ...
    Returns
    -------
    gdf : :class:`geopandas.GeoDataFrame`.  The txt file will be written to the
    output_dir, however the the output gdf itself can be returned from this
