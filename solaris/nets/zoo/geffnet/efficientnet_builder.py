...
class DepthwiseSeparableConv(nn.Module):
    ...
    def forward(self, x):
        ...
        x = checkpoint(lambda y : self.act2(self.bn2(self.conv_pw(self.se(self.act1(self.bn1(self.conv_dw(y))))))), x)

        #x = self.conv_dw(x)
        #x = checkpoint(lambda y : self.se(self.act1(self.bn1(y))), x)
        #x = self.bn1(x)
        #x = self.act1(x)
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        ...
        x = checkpoint(lambda y : self.bn3(self.conv_pwl(self.se(self.act2(self.bn2(self.conv_dw(self.act1(self.bn1(self.conv_pw(y))))))))), x)
        # Point-wise expansion
        #x = self.conv_pw(x)
        #x = checkpoint(lambda y : self.conv_dw(self.act1(self.bn1(y))), x)
        #x = self.bn1(x)
        #x = self.act1(x)

        # Depth-wise convolution
        #x = self.conv_dw(x)
        #x = checkpoint(lambda y : self.se(self.act2(self.bn2(y))), x)
        #x = self.bn2(x)
        #x = self.act2(x)
...
class EfficientNetBuilder:
    ...
    def __call__(self, in_chs, block_args, dilations):
        self.in_chs = in_chs
        ...
        blocks = []
        for stack_idx, stack_args in enumerate(block_args):
            blocks2 = []
            for i, ba in enumerate(stack_args):
                if i >= 1:
                    ba['stride'] = 1

                if ba['block_type'] == 'ir' and dilation > 1:
                    ba['conv_kwargs'] = {'dilation': dilation}
                if ba['stride'] > 1:
                    if dilations[dilation_idx]:
                        dilation *= ba['stride']
                        ba['stride'] = 1
                    dilation_idx += 1

                blocks2.append(self._make_block(ba))
                self.block_idx += 1
            blocks.append(nn.Sequential(*blocks2))
        return blocks
