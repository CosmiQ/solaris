...
def imread(
    path, make_8bit=False, rescale=False, rescale_min="auto", rescale_max="auto"
):
...
def preprocess_im_arr(
    im_arr, im_format, rescale=False, rescale_min="auto", rescale_max="auto"
):
    ...
    if im_arr.ndim not in [2, 3]:
        raise ValueError(
            "This package can only read two-dimensional"
    ...
    if im_format == "uint8":
        ...
        im_arr = (im_arr.astype("float64") * 255.0 / 65535.0).astype("uint8")
...
def scale_for_model(image, output_type=None):
    ...
    if output_type is None:
        ...
        if (image.max() < 255) and (image.max() > 1):
            # scale to min/max
            out_im = 65535.0 * image / 255
            return out_im.astype("uint16")
        elif image.max() <= 1:
            out_im = 65535.0 * image
            return out_im.astype("uint16")
        else:
...
def rescale_arr(im_arr, im_format, rescale_min="auto", rescale_max="auto"):
    ...
    if isinstance(rescale_min, list):
        if len(rescale_min) != im_arr.shape[2]:  # if list len != channels
            raise ValueError(
                "The channel rescaling parameters must be "
    ...
    if scale_factor is not None:
        im_arr = (im_arr - rescale_min) * (scale_factor / (rescale_max - rescale_min))
