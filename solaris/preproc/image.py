...
class MergeToStack(PipeSegment):
    ...
    def transform(self, pin):
        ...
        pout = Image(None, pin[self.main].name, pin[self.main].metadata)
...
class MergeToSum(PipeSegment):
    ...
    def transform(self, pin):
        total = pin[self.main].data.copy()
        for i in range(len(pin)):
            if not i == self.main:
                total += pin[i].data
        return Image(total, pin[self.main].name, pin[self.main].metadata)
...
class MergeToProduct(PipeSegment):
    ...
    def transform(self, pin):
        product = pin[self.main].data.copy()
        for i in range(len(pin)):
            if not i == self.main:
                product *= pin[i].data
        return Image(product, pin[self.main].name, pin[self.main].metadata)
