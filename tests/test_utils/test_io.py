...
class TestPreprocessImArr(object):
    ...
    def test_rescale_auto(self):
        expected_result = np.array(
            [
                [[0, 0, 0], [10, 10, 10], [21, 21, 21], [31, 31, 31], [42, 42, 42]],
                [[53, 53, 53], [63, 63, 63], [74, 74, 74], [85, 85, 85], [95, 95, 95]],
                [
                    [106, 106, 106],
                    [116, 116, 116],
                    [127, 127, 127],
    ...
    def test_rescale_single_vals(self):
        ...
        im_arr = np.arange(5 * 5 * 3, 5 * 5 * 6).reshape(5, 5, 3).astype("uint16")
        normalized_arr = preprocess_im_arr(
            im_arr, "uint16", rescale=True, rescale_min=20, rescale_max=200
        )
    ...
    def test_rescale_limit_range(self):
        expected_result = np.array(
            [
                ...
                [[0, 5, 10], [15, 20, 25], [30, 35, 40], [45, 51, 56], [61, 66, 71]],
                [
                    [76, 81, 86],
                    [91, 96, 102],
                    [107, 112, 117],
        ...
        im_arr = np.arange(5 * 5 * 3, 5 * 5 * 6).reshape(5, 5, 3).astype("uint16")
        normalized_arr = preprocess_im_arr(
            im_arr, "uint16", rescale=True, rescale_min=90, rescale_max=140
        )
