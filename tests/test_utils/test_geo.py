...
class TestGeometriesInternalIntersection(object):
    ...
    def test_with_overlap(self):
        ...
        polygons = poly_df["PolygonWKT_Pix"].apply(lambda x: loads(x).buffer(15)).values
        preds = geometries_internal_intersection(polygons)
...
class TestSplitMultiGeometries(object):
    ...
    def test_simple_split_multipolygon(self):
        output = split_multi_geometries(os.path.join(data_dir, "w_multipolygon.csv"))
        expected = gpd.read_file(os.path.join(data_dir, "split_multi_result.json"))

        assert expected.equals(output)

    def test_grouped_split_multipolygon(self):
        output = split_multi_geometries(
            os.path.join(data_dir, "w_multipolygon.csv"),
            obj_id_col="field_1",
            group_col="truncated",
        )
        expected = gpd.read_file(
            os.path.join(data_dir, "split_multi_grouped_result.json")
        )
...
class TestReproject(object):
    ...
    def test_reproject_rasterio_dataset(self):
        ...
        output = reproject(
            input_data, target_crs=4326, dest_path=os.path.join(data_dir, "tmp.tiff")
        )
        with rasterio.open(input_data) as input_rio:
            input_bounds = input_rio.bounds
            expected_bounds = rasterio.warp.transform_bounds(
                input_rio.crs, "EPSG:4326", *input_bounds
            )
    ...
    def test_reproject_gdf(self):
        ...
        output = reproject(
            input_data, target_crs=4326, dest_path=os.path.join(data_dir, "tmp.json")
        )
        expected_result = gpd.read_file(os.path.join(data_dir, "gt_epsg4326.json"))
...
class TestReprojectGeometry(object):
    ...
    def test_reproject_from_wkt(self):
        ...
        reproj_geom = reproject_geometry(input_str, input_crs=32616, target_crs=4326)
    ...
    def test_reproject_from_wkt_to_utm(self):
        ...
        result_geom = loads(result_str)
        reproj_geom = reproject_geometry(input_str, input_crs=4326, target_crs=32616)
        area_sim = result_geom.intersection(reproj_geom).area / result_geom.area
...
class TestSplitGeometry(object):
    ...
    def test_split_polygon(self):
        ...
        poly = gpd.read_file(os.path.join(data_dir, "test_polygon_split.geojson")).iloc[
            0
        ]["geometry"]
        reproj_poly = reproject_geometry(poly, input_crs=4326, target_crs=32611)
    ...
    def test_split_multigeom_gdf(self):
        ...
        multi_gdf = _check_gdf_load(os.path.join(data_dir, "multigeom.geojson"))
