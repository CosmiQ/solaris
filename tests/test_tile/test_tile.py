...
class TestTilers(object):
    ...
    def test_tiler_custom_proj(self):
        ...
        assert len(raster_tiling_result_files) == len(
            os.listdir(os.path.join(data_dir, "rastertile_test_custom_proj_expected"))
        )
        for f in raster_tiling_result_files:
            result = skimage.io.imread(
                os.path.join(data_dir, "rastertile_test_custom_proj_result", f)
            )
            expected = skimage.io.imread(
    ...
    def test_tiler_fill_nodata(self):
        ...
        raster_tiler = RasterTiler(
            os.path.join(data_dir, "rastertile_test_fill_nodata_result"),
            src_tile_size=(128, 128),
            nodata=-9999.0,
            aoi_boundary=bounds_poly,
        )
        raster_tiler.tile(
            src=os.path.join(data_dir, "nebraska_landsat5_with_nodata_wgs84.tif"),
        ...
        vector_tiling_result_files_nonfilled = os.listdir(
            os.path.join(data_dir, "vectortile_test_nonfilled_result")
        )
        ...
        assert len(vector_tiling_result_files) == len(
            os.listdir(os.path.join(data_dir, "vectortile_test_filled_expected"))
        )
        ...
        assert len(raster_tiling_result_files) == len(
            os.listdir(os.path.join(data_dir, "rastertile_test_fill_nodata_expected"))
        )
        ...
        assert len(vector_tiling_result_files) == len(
            os.listdir(os.path.join(data_dir, "vectortile_test_nonfilled_expected"))
        )

        # check if the filling worked for both img and vector tiles
        for f in raster_tiling_result_files:
            result = skimage.io.imread(
                os.path.join(data_dir, "rastertile_test_fill_nodata_result", f)
            )
            expected = skimage.io.imread(
