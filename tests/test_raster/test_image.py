...
class TestGetGeoTransform(object):
    """Tests for lunuraster.image.get_geo_transform()."""
...
class TestStitchImages(object):
    """Tests for image stitching with lunuraster.image.stitch_images()."""

    def test_stitch_InferenceTiler_output(self):
        # inf_tiler = lunu.nets.datagen.InferenceTiler('keras',
        #                                             width=250, height=250)
        tiles, tile_inds, _ = inf_tiler(os.path.join(data_dir, "sample_geotiff.tif"))
        restored_im = stitch_images(tiles, idx_refs=tile_inds, out_width=900, out_height=900)
        expected_result = lunuutils.io.imread(os.path.join(data_dir, "sample_geotiff.tif"))
    ...
    def test_stitch_firstval(self):
        ...
        result = stitch_images(
            tiles, idx_refs=tile_inds, out_width=900, out_height=900, method="first"
        )
        expected_result = np.load(os.path.join(data_dir, "stitching_first_output.npy"))

        assert np.array_equal(result, expected_result)

    def test_stitch_conf(self):
        src_im = skimage.io.imread(os.path.join(data_dir, "sample_fp_mask_from_geojson.tif"))
        src_im[src_im != 0] = 1
        ...
        result = stitch_images(
            tiles, idx_refs=tile_inds, out_width=900, out_height=900, method="confidence"
        )
        expected_result = np.load(os.path.join(data_dir, "stitching_conf_output.npy"))
